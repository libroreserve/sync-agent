...
IF ERRORLEVEL 1 (
  IF NOT exist vendor\git-2.10.1-32-bit.exe (
    ...
    rem powershell -command "$clnt = new-object System.Net.WebClient; $clnt.DownloadFile(\"https://github.com/libroreserve/sync-agent/blob/downloads/vendor/git-2.10.1-32-bit.exe\", \"c:\libro-sync-agent\vendor\git-2.10.1-32-bit.exe\")"
    powershell -command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -OutFile \"c:\libro-sync-agent\vendor\git-2.10.1-32-bit.exe\" \"https://github.com/libroreserve/sync-agent/blob/downloads/vendor/git-2.10.1-32-bit.exe\""
  )
  @echo Installing Git...
