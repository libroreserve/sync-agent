...
IF NOT exist "!GIT!" (
  ...
  IF NOT exist "vendor\!GIT_INSTALLER!" (
    ...
    powershell -command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -OutFile \"c:\libro-sync-agent\vendor\!GIT_INSTALLER!\" \"https://github.com/opentable/libro-sync-agent/raw/downloads/vendor/!GIT_INSTALLER!\""
    IF NOT exist "vendor\!GIT_INSTALLER!" (
      @echo Invoke-WebRequest failed...
      powershell -command "(New-Object Net.WebClient).DownloadFile(\"https://github.com/opentable/libro-sync-agent/raw/downloads/vendor/!GIT_INSTALLER!\", \"c:\libro-sync-agent\vendor\!GIT_INSTALLER!\")"
    )
    IF NOT exist "vendor\!GIT_INSTALLER!" (
    ...
    IF NOT exist "vendor\!GIT_INSTALLER!" (
      ...
      @echo Please download "https://github.com/opentable/libro-sync-agent/raw/downloads/vendor/!GIT_INSTALLER!", move it to "c:\libro-sync-agent\vendor\" and relaunch this script.
      color c
      timeout 3600
      exit /b
    )
...
IF exist .git (
  ...
  call "!GIT!" pull origin main --force
) ELSE (
  call "!GIT!" init
